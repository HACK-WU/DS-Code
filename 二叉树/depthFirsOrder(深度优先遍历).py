...
# @Time : 2023/3/19 15:04
# @Author : HackWu
# @File : depthFirsOrder(深度优先遍历)
# @Project : DS-Code
...
class BinTree():    #定义二叉树
    ...
    def depthFirstOrder(self,root):     #深度优先遍历,使用递归的方式
        if root==None:return
        print(root.data)
        self.depthFirstOrder(root.left)
        self.depthFirstOrder(root.right)

    def depthFirsOrderStack(self,root): #深度优先遍历，使用非递归的方式
        if root==None:return
        stack=[root]
        while stack:
...
if __name__ == '__main__':
    ...
    tree.depthFirstOrder(tree.root)
    print("非递归方式：")
    tree.depthFirsOrderStack(tree.root)
